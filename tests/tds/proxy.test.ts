...
const baseEnvironment = {
  ...
  TDS_KEY_ID: "saobey_pro-v1",
  TDS_SITE_ID: "saobey_pro",
  TDS_TIMEOUT_MS: "1200",
...
describe("proxy routing", () => {
  it.each(["gclid", "gbraid", "wbraid"])(
    ...
    async (name) => {
      ...
      const response = await runProxy(`https://saobey.pro/?${name}=123abc`);
      const destination = new URL(response.headers.get("location") ?? "");

      expect(response.status).toBe(307);
      expect(destination.pathname).toBe("/from-palladium");
      expect(destination.searchParams.get(name)).toBe("123abc");
      expect(destination.searchParams.get("source")).toBe("saobey.pro");
  ...
  it.each<string>([
    "https://saobey.pro/",
    "https://saobey.pro/?utm_source=google",
    "https://saobey.pro/kontakt?gclid=123abc",
    "https://saobey.pro/?gclid=",
  ...
  it("serves the ordinary site after Palladium deny", async () => {
    ...
    const response = await runProxy("https://saobey.pro/?gclid=123abc");
    expect(response.status).toBe(200);
    expect(response.headers.get("location")).toBeNull();
  ...
  it("forwards Vercel's anti-spoofed client IP and the authentic User-Agent", async () => {
    ...
    const fetchMock = decisionResponse("allow");
    vi.stubGlobal("fetch", fetchMock);
    await runProxy("https://saobey.pro/?gclid=123abc", "AdsBot-Google");
  ...
  it("does not make its own decision depend on User-Agent", async () => {
    ...
    const chrome = await runProxy("https://saobey.pro/?gclid=123abc", "Mozilla/5.0 Chrome/140");
    const crawler = await runProxy("https://saobey.pro/?gclid=123abc", "AdsBot-Google");
  ...
  it("keeps GET and HEAD routing consistent", async () => {
    applyEnvironment();
    vi.stubGlobal("fetch", decisionResponse("allow"));
    const getResponse = await runProxy("https://saobey.pro/?gclid=123abc", "Test Browser", "GET");
    const headResponse = await runProxy("https://saobey.pro/?gclid=123abc", "Test Browser", "HEAD");
  ...
  it("does not route POST requests", async () => {
    applyEnvironment();
    const response = await runProxy("https://saobey.pro/?gclid=123abc", "Test Browser", "POST");
    expect(response.status).toBe(200);
    expect(response.headers.get("location")).toBeNull();
  });
